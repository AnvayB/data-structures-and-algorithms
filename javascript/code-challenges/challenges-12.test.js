...
const validatePin = (pin) => {
  let correctPin = /^[0-9][0-9][0-9][0-9]$/g;
...
const validateWord = (word) => {
  let correctWord = /^[A-Za-z]{5,10}$/g;
  return correctWord.test(word);
};
...
const hasNumber = (string) => {
  let correctNum = /[A-Za-z][0-9]/g;
  return correctNum.test(string);
};
...
const validateEmail = (email) => {
  let correctEmail = /(([a-zA-z0-9])*.?\w*){1,2}@\w*((.com)|(.net)|(.org))/g;
...
const validatePhoneNumber = (phoneNumber) => {
  let correctPhone = /^\(?\d{3}\)? ?-?\d{3}-? ?\d{4}$/g;
  if (correctPhone.test(phoneNumber)) {
    return true;
  } else {
    return false;
  }
