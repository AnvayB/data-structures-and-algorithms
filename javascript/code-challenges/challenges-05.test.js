...
const addValues = (arr) => {
  let sum = arr.reduce(function(adder, value) {
    adder = adder + value;
    return adder;
  }, 0)
  return sum;
...
const addPurchases = (arr) => {
  ...
  let totalPrice = arr.reduce((total, array) => total + array.purchasePrice, 0);
  // let totalPrice = arr.reduce(function(total, array) {
  //   total = total + array.purchasePrice;
  //   return total;
  // }, 0);
  return totalPrice;
};
...
const countNumberOfElements = (arr) => {
  let count = arr.reduce(function (current) {
    return 0 + arr[current];
  });
